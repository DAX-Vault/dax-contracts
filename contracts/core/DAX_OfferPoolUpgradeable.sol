...
/**
 * @title DAX_OfferPoolUpgradeable
 * @notice Permissionless, Non-Custodial Protocol Vault & Open Offer Marketplace (UUPS Upgradeable).
 ...
 */
contract DAX_OfferPoolUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _agreementContract, address _initialOwner) external initializer {
    ...
    function setAgreementContract(address _newAgreementContract) external onlyOwner {
        require(_newAgreementContract != address(0), "DAX_OfferPool: Invalid agreement contract");
        agreementContract = _newAgreementContract;
        emit AgreementContractUpdated(_newAgreementContract);
    }
